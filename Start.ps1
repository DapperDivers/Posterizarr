...
function WatchDirectory {
    ...
    # Initialize the concurrent queue for file processing
    $script:fileQueue = [System.Collections.Concurrent.ConcurrentQueue[string]]::new()
    Write-Host "Initialized file processing queue." -ForegroundColor Magenta
    ...
    try {
        ...
        $onCreated = {
            ...
            $script:fileQueue.Enqueue($path)
            Write-Host "[$timeStamp] Queued file for processing: $path" -ForegroundColor Magenta
        }
        ...
        try {
            ...
            while ($true) {
                ...
                # --- Check and process files from the queue ---
                $filePathToProcess = $null
                if ($script:fileQueue.TryDequeue([ref]$filePathToProcess)) {
                    Write-Host "Dequeued file for processing: $filePathToProcess" -ForegroundColor Magenta
                    # Check if Posterizarr is already running before processing from queue
                    $currentlyRunningFile = "$env:APP_DATA/temp/Posterizarr.Running"
                    $isProcessRunning = (Get-Process | Where-Object commandline -like '*Posterizarr.ps1*')
                    if ((Test-Path $currentlyRunningFile) -or $isProcessRunning) {
                        Write-Host "Watch mode: File processing skipped for '$filePathToProcess' - Posterizarr is already running. Re-queuing." -ForegroundColor Yellow
                        # Re-queue the file to try again later
                        Start-Sleep -Milliseconds 100 # Small delay before re-queuing
                        $script:fileQueue.Enqueue($filePathToProcess)
                    } else {
                        ProcessPosterizarrFile -FilePath $filePathToProcess
                        Write-Host "Watching for .posterizarr files... Press Ctrl+C to stop." -ForegroundColor Yellow
                    }
                }
                ...
                # Sleep briefly before the next loop iteration to prevent busy-waiting
                # Adjust sleep time based on whether the queue was processed or not
                if ($null -ne $filePathToProcess) {
                    # If we processed a file (or tried to), sleep less to check queue again sooner
                    Start-Sleep -Milliseconds 100
                } else {
                    # If queue was empty, sleep longer
